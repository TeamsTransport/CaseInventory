...
and generates a master report with region sheets.
...
class LoblawInventoryProcessor:
    ...
    def _apply_date_filters(self, df: pd.DataFrame, file_path: str) -> pd.DataFrame:
        ...
        # Filter 'Arrived @ Warehouse' dates (currently disabled)
        """
        if 'Arrived @ Warehouse' in df.columns:
        ...
        # 'Scheduled Date' is blank or >= yesterday
        if 'Scheduled Date' in df.columns:
            yesterday = pd.to_datetime(datetime.now().date() - timedelta(days=1))
            df = df[df['Scheduled Date'].isna() | (df['Scheduled Date'] >= yesterday)]

        # Filter rows where 'Original order #' is not blank
        if 'Original order #' in df.columns:
            df = df[df['Original order #'].notna() & (df['Original order #'].astype(str).str.strip() != '')]
    ...
    # --------- NEW: Region sheet helpers ----------
    def _split_into_region_sheets(self, df: pd.DataFrame) -> dict:
        """
        Split the DataFrame into region-specific DataFrames based on the
        'Warehouse Location' column (case-insensitive). Returns a dict:
        {sheet_name: sub_dataframe}
        """
        if 'Warehouse Location' not in df.columns:
            return {}

        loc = df['Warehouse Location']

        # Ordered rules (first match wins)
        rules = [
            ('WOD', loc.str.match(r'^w[123]', case=False, na=False)),            # W1*, W2*, W3* from start
            ('GTA', loc.str.contains(r'gta|tor', case=False, na=False)),         # GTA or TOR
            ('MON', loc.str.contains(r'moncton|warren', case=False, na=False)),  # moncton or warren
            ('PER', loc.str.contains(r'precision', case=False, na=False)),       # Precision
            ('WPG', loc.str.contains(r'wpg', case=False, na=False)),
            ('CGY', loc.str.contains(r'cgy', case=False, na=False)),
            ('VCR', loc.str.contains(r'vcr', case=False, na=False)),
            ('STN', loc.str.contains(r'sas', case=False, na=False)),             # SAS
            ('REG', loc.str.contains(r'reg', case=False, na=False)),
            ('EDM', loc.str.contains(r'edm', case=False, na=False)),
        ]
        ...
        for name, mask in rules:
            final_mask = mask & remaining
            sub = df.loc[final_mask].copy()
            if not sub.empty:
                out[name] = sub
            remaining &= ~mask

        # MISC = anything unmatched (including blanks)
        misc = df.loc[remaining].copy()
        if not misc.empty:
            out['MISC'] = misc

        return out

    def _write_region_sheets(self, df: pd.DataFrame, writer: pd.ExcelWriter) -> None:
        """
        Write region sheets to the Excel file using the provided writer.
        Only writes a sheet if its DataFrame has rows.
        """
        regions = self._split_into_region_sheets(df)
        for sheet_name, subdf in regions.items():
            subdf.to_excel(writer, index=False, sheet_name=sheet_name)

    def save_consolidated_report(self, df: pd.DataFrame, filename: str) -> str:
        """Save the consolidated report with region sheets and formulas on Master."""
        ...
        # Convert relevant columns to datetime (safe)
        if 'Arrived @ Warehouse' in df.columns:
        ...
        columns_to_hide = [
            'Estimated Ship Date', 'Trailer or Warehouse', 'Original Trailer #',
            'Touched / not Touched', 'Date Stripped', 'Delivery Order #', 'Department',
            'LH Gable', 'RH Gable', 'No Gable'
        ]
        df = df.drop(columns=[col for col in columns_to_hide if col in df.columns], errors='ignore')

        # --- Write Master + Region sheets in one pass ---
        with pd.ExcelWriter(output_path, engine='openpyxl', datetime_format='MM-DD-YY') as writer:
            ...
            # Regions
            self._write_region_sheets(df, writer)

        # --- Inject formulas into Master only ---
        ...
        if 'Master' in wb.sheetnames:
            ...
            def has(cols: list[str]) -> bool:
                return all(c in col_indices for c in cols)

            # # Days In Storage = (Storage Ends - Storage Starts) + 1
            if has(['Storage Starts', 'Storage Ends', '# Days In Storage']):
            ...
            # Extended Price = (((Storage Charge * Square Footage of Case)/30) * # Days In Storage) + 60
            if has(['Storage Charge', 'Square Footage of Case', '# Days In Storage', 'Extended Price']):
                for row in range(2, ws.max_row + 1):
...
def main():
    ...
    try:
        ...
        output_filename = f"LoblawsWarehousInventory_{timestamp}.xlsx"
        ...
        # Optional: print region counts
        try:
            regions = processor._split_into_region_sheets(consolidated_data)
            if regions:
                print("\nRows by region:")
                for name in ['WOD', 'GTA', 'MON', 'PER', 'WPG', 'CGY', 'VCR', 'STN', 'REG', 'EDM', 'MISC']:
                    if name in regions:
